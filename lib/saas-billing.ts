...
export async function reconcileSaasWaveTransaction(transactionId: string, checkout: WaveCheckout) {
  ...
  const { data: invoice } = await database.from("saas_invoices").select("id,amount,status").eq("id", transaction.invoice_id).eq("organization_id", transaction.organization_id).maybeSingle();
  ...
  if (transaction.subscription_id) {
    await database.from("saas_subscriptions").update({ status: "active", starts_at: now, updated_at: now }).eq("id", transaction.subscription_id);
  }
  return "succeeded";
}
