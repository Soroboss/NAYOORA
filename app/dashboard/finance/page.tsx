...
export default async function FinancePage() {
  ...
  const { data: membership } = await insforge.from('organization_members')
  ...
  const [plans, members, contributions, payments, pendingPayments, cash] = await Promise.all([
    insforge.from('contribution_plans').select('id,name,amount,frequency').eq('organization_id', org).eq('active', true).order('created_at', { ascending: false }),
    ...
    insforge.from('payments').select('id,amount,paid_at,provider,member:member_profiles(first_name,last_name)').eq('organization_id', org).eq('status', 'completed').order('paid_at', { ascending: false }).limit(8),
