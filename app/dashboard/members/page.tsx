...
export default async function MembersPage() {
  ...
  const { data: members } = await insforge.from("member_profiles").select("id,first_name,last_name,phone,email,member_number,status,birth_date,address,photo_url").eq("organization_id", membership.organization_id).is("deleted_at", null).order("last_name");
  return <main className="app-shell"><aside className="sidebar"><Link href="/dashboard" className="brand"><img src="/nayoora-logo.png" alt="" /> NAYOORA</Link><div className="org-switch"><span>◉</span><div><b>{(membership.organization as unknown as { name: string } | null)?.name}</b><small>Espace organisation</small></div></div><nav><Link href="/dashboard">◈ Vue d'ensemble</Link><Link className="active" href="/dashboard/members">· Membres</Link><Link href="/dashboard/finance">· Cotisations</Link><Link href="/dashboard/treasury">· Caisse</Link><Link href="/dashboard/reports">· Rapports</Link></nav></aside><section className="dashboard"><header className="dashboard-header"><div><p className="eyebrow">Répertoire</p><h1>Membres</h1><p>Gérez les personnes qui font vivre votre organisation.</p></div></header><MembersManager organizationId={membership.organization_id} members={(members ?? []) as Member[]} canManage={["organization_admin","president","secretaire","gestionnaire"].includes(membership.role)} /></section></main>;
}
