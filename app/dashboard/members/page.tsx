...
export default async function MembersPage() {
  ...
  const { data: membership } = await supabase.from("organization_members").select("organization:organizations(name),role,organization_id").eq("user_id", user.id).eq("status", "active").limit(1).maybeSingle(); if (!membership) redirect("/onboarding");
  const { data: members } = await supabase.from("member_profiles").select("id,first_name,last_name,phone,email,member_number,status,birth_date,address").eq("organization_id", membership.organization_id).is("deleted_at", null).order("last_name");
