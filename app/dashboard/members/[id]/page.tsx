...
export default async function MemberProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: member } = await insforge.from("member_profiles").select("id,organization_id,first_name,last_name,phone,email,address,member_number,status,birth_date,joined_on,photo_url,office_role,office_title,role_started_on,elected_until,is_current_officer,member_cards(id,status,front_image_url,back_image_url,pdf_url,expires_at)").eq("organization_id", membership.organization_id).eq("id", id).is("deleted_at", null).maybeSingle();
  if (!member) notFound();
