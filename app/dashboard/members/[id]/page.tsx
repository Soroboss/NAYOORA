...
export default async function MemberProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: cardSettings } = await insforge.from("settings").select("*").eq("organization_id", membership.organization_id).maybeSingle();
  const settingsData = cardSettings?.data || {};
  const finalSettings = {
    primary_color: settingsData.card_primary_color || '#1e40af',
    theme: settingsData.card_theme || 'modern',
  };
