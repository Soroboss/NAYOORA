...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "role") { if (!body.membershipId || !validRoles.includes(body.role)) throw new Error("Rôle invalide."); const { data: target } = await insforge.from("organization_members").select("role").eq("id", body.membershipId).eq("organization_id", organizationId).maybeSingle(); if (!target) throw new Error("Utilisateur introuvable."); if (!administratorRoles.includes(target.role) && administratorRoles.includes(body.role)) await enforceAdministratorLimit(); const { data, error } = await insforge.from("organization_members").update({ role: body.role }).eq("id", body.membershipId).eq("organization_id", organizationId).select().single(); if (error) throw error; return NextResponse.json({ item: data }); }
    if (body.action === "invite") { if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email || "")) throw new Error("Email invalide."); const role = validRoles.includes(body.role) ? body.role : "membre"; if (administratorRoles.includes(role)) await enforceAdministratorLimit(); const { data, error } = await insforge.from("organization_invites").insert({ organization_id: organizationId, email: body.email.toLowerCase(), role, invited_by: user.id }).select().single(); if (error) throw error; return NextResponse.json({ item: data }); }
