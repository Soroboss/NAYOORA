...
export async function POST(request: NextRequest) {
  ...
  if (mode === "verify") {
    ...
    const redirectTo = await getAuthDestination(result.data.accessToken);
  ...
  const redirectTo = isSignupLike(mode) ? "/onboarding" : await getAuthDestination(result.data.accessToken);
  const response = NextResponse.json({ ok: true, redirectTo });
  response.cookies.set("insforge_access_token", result.data.accessToken, { ...authCookieOptions, maxAge: 60 * 15 });
...
function isSignupLike(mode: unknown) {
  return mode === "signup";
}
