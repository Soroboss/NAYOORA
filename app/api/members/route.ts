...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await ctx.supabase.from("member_profiles").insert({ organization_id: ctx.membership.organization_id, first_name: member.firstName, last_name: member.lastName, phone: member.phone, email: member.email, address: member.address, member_number: member.memberNumber, birth_date: member.birthDate, created_by: ctx.user.id }).select().single();
...
export async function DELETE(request: Request) {
  ...
  const { error } = await ctx.supabase.from("member_profiles").update({ status: "inactive", deleted_at: new Date().toISOString() }).eq("id", id).eq("organization_id", ctx.membership.organization_id).is("deleted_at", null);
