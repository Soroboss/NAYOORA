...
export async function POST(request: Request) {
  ...
  const { data: membership } = user ? await supabase.from("organization_members").select("organization_id,role").eq("user_id", user.id).eq("status", "active").limit(1).maybeSingle() : { data: null };
  ...
  try {
    const rows = parseCsv(await file.text()); const { data: batch, error: batchError } = await supabase.from("member_imports").insert({ organization_id: membership.organization_id, file_name: file.name, total_rows: rows.length, imported_by: user.id }).select("id").single();
    ...
    let imported = 0; if (valid.length) { const { error } = await supabase.from("member_profiles").insert(valid); if (error) throw error; imported = valid.length; }
    if (errors.length) await supabase.from("member_import_errors").insert(errors);
    const status = errors.length ? "completed_with_errors" : "completed"; await supabase.from("member_imports").update({ imported_rows: imported, failed_rows: errors.length, status, completed_at: new Date().toISOString() }).eq("id", batch.id);
