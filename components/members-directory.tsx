...
export function MembersDirectory({ members: initialMembers, canManage }: { members: DirectoryMember[]; canManage: boolean }) {
  ...
  return <div className="members-workspace"><div className="directory-toolbar"><div><p className="eyebrow">{members.length} membre{members.length > 1 ? "s" : ""}</p><h2>Liste des membres</h2></div><div><input aria-label="Rechercher un membre" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher…" />{canManage && <Link className="button button-dark" href="/dashboard/members/new">+ Créer un membre</Link>}</div></div>{message && <p className="member-message">{message}</p>}<article className="panel directory"><div className="member-list">{filtered.map((member) => <div className="member-row member-row-wide" key={member.id}><span className="member-avatar">{member.photo_url ? <img src={member.photo_url} alt="" /> : `${member.first_name[0]}${member.last_name[0]}`}</span><div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}><div style={{ display: 'flex', alignItems: 'center' }}><b>{member.first_name} {member.last_name}</b><WhatsAppButton phone={member.phone} message={`Bonjour ${member.first_name} ! J'espère que tu vas bien.`} /></div><small>Matricule: {member.member_number || "Auto"} • {member.phone || member.email || "Aucun contact"}</small></div><span className={`member-status ${member.status}`}>{member.status === "active" ? "Actif" : member.status}</span><span className="member-office-chip">{roleLabel[member.office_role || "member"] ?? member.office_role}{member.office_title ? ` · ${member.office_title}` : ""}</span><div className="row-actions"><Link href={`/dashboard/members/${member.id}`} className="button" style={{padding: '6px 12px', background: '#e3edbd', color: '#16372b'}}>Modifier / Fiche</Link>{canManage && <button onClick={() => remove(member)}>Retirer</button>}</div></div>)}{!filtered.length && <div className="empty-state"><div>◎</div><h3>Aucun membre trouvé.</h3><p>Créez un membre ou ajustez votre recherche.</p></div>}</div></article></div>;
}
