...
const services = [
  ["essentiel", "Essentiel", "Membres, cotisations et communication"],
  ["pilotage", "Pilotage", "Finances, caisse et activités"],
  ["complet", "Complet", "Tous les modules adaptés"],
] as const;

export function OnboardingForm() {
  const router = useRouter(); const [selected, setSelected] = useState<OrganizationType>("mutuelle"); const [service, setService] = useState<(typeof services)[number][0]>("pilotage"); const [name, setName] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  useEffect(() => { const saved = sessionStorage.getItem("nayoora-onboarding"); if (!saved) return; try { const value = JSON.parse(saved); setSelected(value.organizationType ?? "mutuelle"); setService(value.service ?? "pilotage"); } catch {} }, []);
