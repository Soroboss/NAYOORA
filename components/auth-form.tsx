...
const serviceChoices = [
  ...
  ["complet", "Complet", "Tous les modules adaptés à votre organisation"],
] as const;

export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  const router = useRouter(); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  const [organizationType, setOrganizationType] = useState<(typeof organizationChoices)[number][0]>("association"); const [service, setService] = useState<(typeof serviceChoices)[number][0]>("pilotage");
